...
#Import records
occ <- fread("Check_Points/4-Flagged_Records.csv")
#Get all species
...
with_progress({
  p <- progressor(along=seq_along(spp))
  occ_flags <- future_lapply(seq_along(spp), FUN = function(i){
    tryCatch(
      {#Identify species
        sp <- spp[i]
        #Subset records of the specie
        pts <- occ %>% filter(species == sp)
        ...
        occ.dup <- cbind(occ.zero, "Duplicated" = occ.dup)
        return(occ_dup)
        #Print Progress
        p(sprintf("i=%g", i))}, 
...
occ.final$Inside_BR[which(occ.final$Inside_BR == FALSE)] <- "Endemica_out_BR"
