...
#Here, we also included records from 3 other sources:
...
l.spp <- read.csv("List_of_species.csv")
l.spp <- unique(l.spp$species)

#Remember: as example, we inclued in our directory only records from Araucaria angustifolia
...
clusterExport(cl, varlist= ls() %>% dput(), #Get all objects created untill now
              envir=environment())
clusterEvalQ(cl, {
  library(plantR)
  library(data.table)
...
pblapply(seq_along(l.spp), FUN = function(i){
  tryCatch(
    {#Identify species
      sp <- l.spp[i]
